...
const workspace = await client.sandboxes.create({
  mounts: [
    ...
    { disk: "dsk-def456", path: "/mnt/models", subdirectory: "llama", readOnly: true },
    { disk: "dsk-789abc", path: "/workspace", conditional: true },
  ],
});
console.log(workspace.mounts.map((mount) => mount.path));

await workspace.stop();
await workspace.start({ mounts: [{ disk: "dsk-abc123", path: "/mnt/other" }] }); // replace them for the next session
```
