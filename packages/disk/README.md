...
to a non-terminal process. `sendInput()` splits large input into 1 MiB chunks,
waits for each chunk to be accepted, and retries when the sandbox applies
backpressure. Processes end when their sandbox is stopped, paused, or expires.
