...
Sandboxes are long-lived microVMs with a dedicated persistent Archil root disk. Unlike `exec` (one container per command), a sandbox stays up between commands and keeps its filesystem across cold starts.
...
const result = await sandbox.run("pwd && ls -la");
console.log(result.exitCode, result.stdout);

await sandbox.pause();
const fork = await sandbox.fork({ name: "experiment-b" });
const connection = await fork.connection();
console.log(connection.url, connection.expiresAt);

await sandbox.resume();
await sandbox.stop();

const sandboxes = await sdk.sandbox.list();
const restarted = await sdk.sandbox.start({ id: sandboxes[0].id });
```

`create`, `start`, `stop`, `pause`, `resume`, and `fork` wait for the target state by default (pass `wait: false` to just submit). `run` waits until the command finishes; a non-zero exit code is reported in the result, not thrown. `connection` returns a signed WebSocket URL and its expiration time. Module-level equivalents (`createSandbox`, `getSandbox`, `listSandboxes`, `startSandbox`) use the `configure`d client.
