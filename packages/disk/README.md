...
`readOnly`, `conditional` (concurrent writers without delegation checkouts), `subdirectory`,
`queueMs`, and `checkoutPaths`. A plain read-write mount takes the disk's exclusive root
delegation for as long as the sandbox holds it, including while paused, so use `readOnly`
