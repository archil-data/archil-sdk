...
// Vercel AI SDK. A model served by Ollama actually *chooses and calls* our
// tools through generateText's agent loop, against an in-memory disk (no Archil
...
const { Archil } = await import(path.join(root, "dist/index.js"));
const { agentTools } = await import(path.join(root, "dist/ai-sdk.js"));

// --- in-memory disk via a stubbed global fetch (no real Archil backend) ---
const store = {};
globalThis.fetch = async (input, init = {}) => {
  ...
  if (u.host === "cp.test") {
    return new Response(
      JSON.stringify({
        success: true,
        data: { id: "dsk-1", name: "smoke", organization: "o", status: "available", provider: "aws", region: "aws-us-east-1", createdAt: "2026-01-01T00:00:00Z" },
      }),
      { headers: { "content-type": "application/json" } },
    );
  }
  const key = decodeURIComponent(u.pathname.split("/").slice(2).join("/"));
  if (method === "PUT") { store[key] = await req.text(); return new Response(null, { status: 200, headers: { etag: '"x"' } }); }
  if (method === "GET" && key === "") {
    const items = Object.keys(store).sort().map((k) => `<Contents><Key>${k}</Key><Size>${store[k].length}</Size></Contents>`).join("");
    return new Response(`<?xml version="1.0"?><ListBucketResult><IsTruncated>false</IsTruncated><KeyCount>1</KeyCount>${items}</ListBucketResult>`, { status: 200 });
  }
  if (method === "GET" || method === "HEAD") {
    return key in store ? new Response(store[key], { status: 200 }) : new Response("<Error><Code>NoSuchKey</Code></Error>", { status: 404 });
  }
  return new Response(null, { status: 400 });
};

const archil = new Archil({ apiKey: "key-smoke", region: "aws-us-east-1", baseUrl: "http://cp.test", s3BaseUrl: "http://s3.test" });
...
let finalText = "";
try {
  const res = await generateText({ model: ollama(MODEL), tools: agentTools(disk), stopWhen: stepCountIs(8), prompt });
  finalText = res.text;
...
if ((store["notes.txt"] ?? "").includes("hello from a real model")) {
  ...
  console.log(
    "\n⚠️  SMOKE INCONCLUSIVE — the model didn't produce a clean write_file call " +
