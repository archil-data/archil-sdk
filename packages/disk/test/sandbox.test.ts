...
function sandboxWire(overrides: Record<string, unknown> = {}) {
  return {
    ...
    platform: "arm64",
    endpoints: [{ port: 8080, hostname: "sb-1.example.test" }],
...
test("create maps options to the wire request and waits until running", async () => {
  ...
  await withStub(
    ...
    async (archil, calls) => {
      ...
      assert.equal(calls[0].search, "?wait=true");
      ...
      assert.equal(sandbox.info.name, "sandbox-one");
      assert.deepEqual(sandbox.info.endpoints, [
        { port: 8080, hostname: "sb-1.example.test" },
      ]);
      assert.equal(polls, 2);
      assert.ok(calls.slice(1).every((call) => call.search === ""));
    },
  );
});

test("create sends the generated image default", async () => {
  await withStub(
    () => envelope(sandboxWire(), 202),
    async (archil, calls) => {
      await archil.sandbox.create();
      assert.deepEqual(calls[0].body, { base_image: "ubuntu:26.04" });
      assert.equal(calls[0].search, "?wait=true");
    },
  );
});

test("create can return without waiting", async () => {
  await withStub(
    () => envelope(sandboxWire({ status: "pending" }), 202),
    async (archil, calls) => {
      const sandbox = await archil.sandbox.create({ wait: false });
      assert.equal(sandbox.status, "pending");
      assert.equal(calls.length, 1);
      assert.equal(calls[0].search, "?wait=false");
    },
  );
});
...
test("run submits the exec and polls until terminal", async () => {
  ...
  await withStub(
    ...
    async (archil, calls) => {
      ...
      const result = await sandbox.run("ls", {
        env: { A: "1" },
        timeoutMs: 30_000,
        ...fastPoll,
      });
      ...
      assert.equal(submit?.search, "?wait=true");
      ...
      assert.ok(
        calls.filter((call) => call.path.includes("/execs/")).every((call) => call.search === ""),
      );
    },
  );
});
...
test("stop waits for the sandbox to become inactive", async () => {
  ...
  await withStub(
    ...
    async (archil, calls) => {
      ...
      assert.ok(calls.filter((call) => call.method === "GET").every((call) => call.search === ""));
      assert.equal(sandbox.status, "stopped");
    },
  );
});

test("pause waits for the memory snapshot to become durable", async () => {
  let refreshes = 0;
  await withStub(
    (call) => {
      if (call.method === "POST") return envelope(sandboxWire({ status: "pausing" }), 202);
      refreshes += 1;
      return envelope(sandboxWire({ status: refreshes === 1 ? "running" : "paused" }));
    },
    async (archil, calls) => {
      const sandbox = await archil.sandbox.get("sb-1");
      await sandbox.pause(fastPoll);
      assert.ok(calls.some((call) => call.path === "/api/sandboxes/sb-1/pause"));
      assert.equal(sandbox.status, "paused");
    },
  );
});

test("resume restores a paused sandbox", async () => {
  let refreshes = 0;
  await withStub(
    (call) => {
      if (call.method === "POST") return envelope(sandboxWire({ status: "pending" }), 202);
      refreshes += 1;
      return envelope(sandboxWire({ status: refreshes === 1 ? "paused" : "running" }));
    },
    async (archil, calls) => {
      const sandbox = await archil.sandbox.get("sb-1");
      await sandbox.resume(fastPoll);
      const resume = calls.find((call) => call.path.endsWith("/resume"));
      assert.equal(resume?.search, "?wait=true");
      assert.equal(sandbox.status, "running");
    },
  );
});

test("fork returns a running child sandbox", async () => {
  await withStub(
    (call) => {
      if (call.path.endsWith("/fork")) {
        return envelope(
          sandboxWire({ sandbox_id: "sb-child", name: "experiment-b", status: "pending" }),
          202,
        );
      }
      if (call.path === "/api/sandboxes/sb-child") {
        return envelope(sandboxWire({ sandbox_id: "sb-child", name: "experiment-b" }));
      }
      return envelope(sandboxWire());
    },
    async (archil, calls) => {
      const sandbox = await archil.sandbox.get("sb-1");
      const fork = await sandbox.fork({ name: "experiment-b", ...fastPoll });
      const request = calls.find((call) => call.path.endsWith("/fork"));
      assert.equal(request?.search, "?wait=true");
      assert.deepEqual(request?.body, { name: "experiment-b" });
      assert.equal(fork.id, "sb-child");
      assert.equal(fork.status, "running");
    },
  );
});

test("connection returns signed WebSocket connection info", async () => {
  await withStub(
    (call) =>
      call.path.endsWith("/connections")
        ? envelope({
            url: "wss://session.example.test/ws?token=signed",
            expires_at: "2026-07-14T00:01:00Z",
          })
        : envelope(sandboxWire()),
    async (archil, calls) => {
      const sandbox = await archil.sandbox.get("sb-1");
      const connection = await sandbox.connection();
      assert.ok(calls.some((call) => call.path.endsWith("/connections")));
      assert.equal(connection.url, "wss://session.example.test/ws?token=signed");
      assert.equal(connection.expiresAt.toISOString(), "2026-07-14T00:01:00.000Z");
    },
  );
});
