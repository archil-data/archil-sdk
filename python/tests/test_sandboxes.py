...
async def test_port_tokens_sync_and_async(archil, router):
    metadata = {"id": "token-1", "port": 8080, "created_at": "2026-09-17T00:00:00Z", "expires_at": "2026-09-17T01:00:00Z"}
    responses = iter([
        ok_envelope(sandbox_json()),
...
async def test_port_token_pagination(archil, router):
    def handler(request):
        ...
        return ok_envelope(
            {"tokens": [{"id": "token-2" if second_page else "token-1", "port": 8080, "created_at": "2026-09-17T00:00:00Z"}]},
    ...
    resumed = [page async for page in sandbox.list_port_token_pages.aio(cursor=pages[0].next_cursor)]
    assert [token.id for token in resumed[0].tokens] == ["token-2"]
    assert [r.query for r in router.requests[1:]] == [
        {"limit": "100"}, {"limit": "100", "cursor": "token-1"}, {"limit": "1"},
        {"limit": "1"}, {"limit": "1", "cursor": "token-1"}, {"limit": "100", "cursor": "token-1"},
    ]
