...
def test_fork_returns_child_when_source_cannot_be_resumed(archil, router, monkeypatch):
    import archil._sandbox as sandbox_module
    ...
    def handler(request):
        ...
        if request.url.path.endswith("/resume"):
            return error_envelope(409, "no runtime host available")
        return ok_envelope(sandbox_json("paused"))

    router.set(handler)
    source = archil.sandboxes.get("sbx-1")
    fork = source.fork()
...
def test_fork_resumes_source_when_pause_wait_fails(archil, router, monkeypatch):
    import archil._sandbox as sandbox_module
    ...
    def handler(request):
        ...
        if request.method == "GET":
            reads += 1
            return ok_envelope(sandbox_json()) if reads == 1 else error_envelope(400, "boom")
        return ok_envelope(sandbox_json("pending"))
...
@pytest.mark.asyncio
async def test_run_and_attach_with_streamed_input(archil, router, monkeypatch):
    import archil._sandbox as sandbox_module
    ...
    resumed = await sandbox.attach.aio(process.id, offset=cursor)
...
@pytest.mark.asyncio
async def test_disk_connect_opens_a_fresh_bash_session_on_every_call(archil, router, monkeypatch):
    import archil._sandbox as sandbox_module
    ...
    new_shell = await disk.connect.aio()
    assert json.loads(sockets[1].sent[0]) == {
        "type": "start", "command": "exec /bin/bash -i", "terminal": {"cols": 80, "rows": 24},
        "env": {"TERM": "xterm-256color"},
    }
    await new_shell.disconnect.aio()
    assert [req.json for req in router.requests if req.path == "/api/disks/dsk-1/connect"] == [
        None, None,
    ]
    assert [req.path for req in router.requests if req.path.endswith("/connections")] == [
        "/api/sandboxes/session-1/connections", "/api/sandboxes/session-2/connections",
    ]
