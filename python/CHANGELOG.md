...
- Add `mounts` to sandbox creation so Archil disks are mounted inside the guest at chosen paths, with the same read-only, conditional, subdirectory, and delegation checkout options as `exec`.
