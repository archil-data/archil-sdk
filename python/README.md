...
`path` is required when more than one disk is mounted. Mount options match `disk.exec`:
`read_only`, `conditional` (concurrent writers without delegation checkouts), `subdirectory`,
`queue_ms`, and `checkout_paths`. A plain read-write mount takes the disk's exclusive root
delegation for as long as the sandbox holds it, including while paused, so use `read_only`
