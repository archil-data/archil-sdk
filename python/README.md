...
unmatched traffic does not delay draining. Only HTTP ports 80 and 443 are supported.
...
passthrough connections. An explicit pause that cannot drain in time leaves the
sandbox running and raises `SandboxPauseError` when waiting. Inspect `error.latest`
