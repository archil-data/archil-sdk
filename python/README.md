    ...
    network=archil.SandboxNetwork(
        egress=archil.SandboxEgressPolicy(
            ...
            allow=[
                ...
                "*.github.com",
                "140.82.112.0/20",
                archil.SandboxEgressRule(
                    target="api.openai.com",
                    transform=archil.SandboxEgressTransform(
...
```python
from archil import SandboxEgressPolicy, SandboxNetwork
...
do not grant network access. Pausing gates
new matches and waits for active responses, including streams, to reach the guest;
traffic to other hosts does not delay draining. The drain waits up to ten minutes by default,
starting when pause begins. Request errors stop counting; expiry proceeds with the snapshot.
Requests have no added timeout during normal operation. Only HTTP ports 80 and 443 are supported.
...
passthrough connections. A snapshot failure leaves the sandbox in `failed` with the
