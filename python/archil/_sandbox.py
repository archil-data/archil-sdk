...
class _Sandbox:
    ...
    async def fork(self, *, name: Optional[str] = None, wait: bool = True) -> "_Sandbox":
        ...
        returned even if the source could not be resumed; refresh the source
        afterwards if its state matters, as with every lifecycle call."""
        ...
        try:
            await source._wait_while("pausing")
            data = await self._transport.request_json(
                "POST",
        ...
        if resumed is not None:
            with contextlib.suppress(Exception):
                await resumed._wait_while("pending")
        return sandbox
